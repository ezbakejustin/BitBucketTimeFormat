var $jq = function(selector) {
    return new $jq.prototype.init(selector);
};

$jq.prototype = {
    jquery: "0.1 Barebones Edition",
    length: 0,
    init: function (selector) {
        if (typeof selector === "function") {
            window.onload = selector;
            this[0] = document;
            this.length = 1;
        } else if (selector) {
            if (/^\w+$/.test(selector)) { // tagname
                selector = document.getElementsByTagName(selector);
                return $jq.merge(this, selector);
            } else { // #id
                var id = selector.substr(1);
                var elem = document.getElementById(id);
                if (elem) {
                    this[0] = elem;
                    this.length = 1;
                }
            }
        }
        return this;
    }
}

$jq.each = function(object, callback) {
    for (var i = 0, length = object.length, value = object[0]; i < length; value = object[++i]) {
        callback.call(value, i, value);
    }
    return object;
}

$jq.trim = String.prototype.trim ? function(text) {
    return text == null ? "" : String.prototype.trim.call(text);
} : function (text) {
    return text == null ? "" : text.toString().replace(/^\s+/, "").replace(/\s+$/, "");
};

$jq.merge = function(first, second) {
    var i = first.length,
        j = 0;
    while (second[j] !== undefined) {
        first[i++] = second[j++];
    }
    first.length = i;
    return first;
}

$jq.prototype.init.prototype = $jq.fn = $jq.prototype;
$ = $jq;

$jq.fn.hide = function() {
    return $jq.each(this, function () {
        this.style.display = 'none';
    });
};

$jq.fn.show = function() {
    return $jq.each(this, function () {
        this.style.display = '';
    });
};

$jq.fn.click = function(fn) {
    return $jq.each(this, function () {
        this.onclick = fn;
    });
};

$jq.fn.addClass = function(value) {
    var classNames, i, l, elem
    if (value && typeof value === "string") {
        for (i = 0, l = this.length; i < l; i++) {
            elem = this[i];
            if (elem.nodeType === 1) {
                classNames = elem.className.split(' ');
                if (classNames.indexOf(value) == -1) {
                    classNames.push(value);
                }
            }
            elem.className = classNames.join(' ');
        }
    }
    return this;
};

$jq.fn.removeClass = function(value) {
    var classNames, i, l, elem, ind;
    if (value && typeof value === "string") {
        for (i = 0, l = this.length; i < l; i++) {
            elem = this[i];
            if (elem.nodeType === 1) {
                classNames = elem.className.split(' ');
                ind = classNames.indexOf(value);
                if (ind > -1) {
                    classNames.splice(ind, 1);
                }
            }
            elem.className = classNames.join(' ');
        }
    }
    return this;
};